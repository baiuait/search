$(function () {
    ...
    $('.add_url #btnAddCusNav').click(function () {
        ...
        setTimeout(function(){
            ...
            //保存入数据库
            $.post("urls/addUrl", "url=" + url + "&title=" + title + "&img=" + imgUrl + "&userId=" + $("#checkLoginInput").val(), function (data) {
                if (data.toString() != "-1") {
                    //jQuery方式添加结构
    ...
    $('.note').keydown(function () {
        ...
        if ($('.note_history .active').text() == '') {
            ...
            //异步添加便笺
            $.post("memo/addMemo", "userId=" + $("#checkLoginInput").val() + "&content=" + noteVal, function (data) {
                if (data.toString() != "-1") {
                    //创建新便笺 .active
        ...
        //判断该active是否为top_one_note
        if ($('.note_history .active').hasClass('top_one_note')) {
            $('.top_note').find('.content').text($('.note_history .active').find('.content').text().trim());
        }
    ...
    $('.note_icons .bell').click(function () {
        ...
        //异步修改数据库的便笺置顶属性
        $.post("memo/modifyMemoIsTop", "userId=" + $("#checkLoginInput").val() + "&id=" + $('.note_history .active').attr("id"), function (data) {
            if (data.toString() == "true") {
    ...
    $('.top_note span').click(function () {
        ...
        $.post("memo/cleanTop", "userId="+$("#checkLoginInput").val(),function(){
    ...
    //
    // //判断置顶便笺是否需要显示
    // function checkTopNote() {
    //     if ($('.top_note .content').text().trim() != '') {
    //         $('.top_note').show();
    //     }
    // }
